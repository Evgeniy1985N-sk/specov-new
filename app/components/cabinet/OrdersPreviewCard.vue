...
<template>
  <div class="bg-(--Base-White) rounded-xl p-5">
    <div class="flex items-center justify-between mb-5">
      ...
      <NuxtLink :to="`${route.path.startsWith('/cabinet-corporate') ? '/cabinet-corporate' : '/cabinet-individual'}/orders`" class="flex items-center gap-1 text-sm text-(--Text-950) font-semibold border border-(--border) py-2 px-4 bg-gray-100 rounded-lg">
        Все
    ...
    <div class="flex flex-col gap-2">
      <div v-for="(order, index) in orders" :key="index" class="flex items-center justify-between py-3 bg-gray-100 p-4 rounded-2xl">
        <div>
