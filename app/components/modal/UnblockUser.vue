    ...
    <div class="bg-white rounded-xl p-6 relative">
      ...
      <div class="flex flex-col items-start mb-4 pr-8">
        <div class="w-10 h-10 flex items-center justify-center mb-3">
          <img src="/image/cabinet/reverse-left.svg" alt="" class="w-10 h-10" style="filter: brightness(0) saturate(100%) invert(20%) sepia(98%) saturate(1234%) hue-rotate(120deg) brightness(95%) contrast(85%);">
        </div>
        <h2 class="text-2xl font-regular text-(--Text-950) font-['Russo_One']">
          Вы точно хотите восстановить пользователя?
        </h2>
      </div>

      <p class="text-base text-(--Text-950) mb-6">
        Пользователь <span class="font-bold">{{ userName }}</span> будет восстановлен. После восстановления пользователь появится в разделе <span class="font-bold">«Активные»</span>.
      </p>
